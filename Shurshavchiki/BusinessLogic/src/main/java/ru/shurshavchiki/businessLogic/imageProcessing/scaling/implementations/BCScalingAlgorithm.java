...
public class BCScalingAlgorithm extends ScalingAlgorithmBase {
    ...
    public List<ScalingAlgorithmParameter> getParametersToInit() {
        return List.of(new FloatScalingAlgorithmParameter("B", 0, 1),
                new FloatScalingAlgorithmParameter("C", 0, 1));
    }
    ...
    private RgbConvertable calculateColor(RgbConvertable pixel1, RgbConvertable pixel2, double dist) {
        return new RgbPixel(
...
//        return (float) ((value1 * dist2 + value2 * dist1) / (dist1 + dist2));
        return (12 - (9 * parameterB) - (6 * parameterC)) * dist1
    }
}
